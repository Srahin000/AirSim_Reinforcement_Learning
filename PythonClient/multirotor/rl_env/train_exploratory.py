#!/usr/bin/env python3
"""
Exploratory Training Script for Mountain Pass Environment with Lidar
Training with random goals within increasing exploration radius.
"""
...
def make_env():
    """Create the exploratory environment with monitoring."""
    return Monitor(MountainPassExploratoryEnv(
        ...
        initial_exploration_radius=10.0,  # Start with 10m radius
        max_exploration_radius=50.0,  # Max 50m radius
        radius_increase_episodes=10000  # Increase radius every 10k episodes
    ))

import numpy as np

class ExploratoryTrainingCallback(BaseCallback):
    """Custom callback for exploratory training monitoring with detailed episode logging."""
    def __init__(self):
        super().__init__()
        ...
        self.recent_rewards = []
        self.recent_lengths = []
        self.exploration_radius_history = []  # Track exploration radius changes
    ...
    def _on_step(self):
        ...
        for info in infos:
            ...
            if ep:
                ...
                # Track exploration radius
                current_radius = info.get('exploration_radius', 0)
                if current_radius not in self.exploration_radius_history:
                    self.exploration_radius_history.append(current_radius)
                ...
                print(f"[EPISODE {self.episode_count:4d}] "
                      ...
                      f"Radius: {current_radius:4.1f}m, "
    ...
    def _log_running_statistics(self):
        ...
        print("\n" + "=" * 80)
        print(f"EXPLORATORY TRAINING STATISTICS (Episode {self.episode_count})")
        print("=" * 80)
        ...
        print(f"  Exploration Radius History: {self.exploration_radius_history}")
        ...
        self.logger.record("exploration/current_radius", self.exploration_radius_history[-1] if self.exploration_radius_history else 0)
        self.logger.dump(self.num_timesteps)

class CheckpointCallback(BaseCallback):
    """Callback to save model checkpoints."""
    def __init__(self, save_freq=5000, save_path="./checkpoints/", name_prefix="mountain_exploratory_model"):
        super().__init__()
...
def main():
    ...
    print("[INFO] Starting exploratory training setup...")
    
    # Test environment first
    print("[INFO] Testing exploratory environment...")
    test_env = make_env()
    ...
    for i in range(5):
        ...
        print(f"[TEST] Step {i+1}: Reward={reward:.2f}, Distance={info.get('distance_to_goal', 0):.2f}, "
              f"Collisions={info.get('collision_count', 0)}, Radius={info.get('exploration_radius', 0):.1f}m")
    ...
    latest_model_path = "checkpoints/mountain_exploratory_model_latest"
    
    # Set up logger with enhanced PPO logging
    new_logger = configure("ppo_exploratory_tensorboard/PPO_8", ["tensorboard", "stdout"])
    ...
    if os.path.exists(latest_model_path + ".zip"):
        ...
        print("[INFO] Starting fresh exploratory training...")
        loaded_timesteps = 0
        ...
        model = PPO(
            ...
            vf_coef=0.5,
            max_grad_norm=0.5,
            tensorboard_log="./ppo_exploratory_tensorboard/",
            policy_kwargs=dict(
    ...
    # Create callbacks
    training_callback = ExploratoryTrainingCallback()
    checkpoint_callback = CheckpointCallback(save_freq=1000, save_path='./checkpoints/', name_prefix='mountain_exploratory_model')
    
    # Combine callbacks
    callback = CallbackList([training_callback, checkpoint_callback])
    
    print("[INFO] Starting exploratory training...")
    print("[INFO] Goal: Navigate to random goals within increasing exploration radius")
    print("[INFO] Actions: forward, left, right, up, down")
    print("[INFO] Exploration radius starts at 10m and increases every 10k episodes")
    print("[INFO] Checkpoints will be saved every 1000 steps in ./checkpoints/")
    print("[INFO] TensorBoard logs available at ./ppo_exploratory_tensorboard/")
    ...
    print("[INFO] Exploratory training and testing completed!")
