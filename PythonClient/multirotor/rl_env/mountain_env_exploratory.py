...
class MountainPassExploratoryEnv(gym.Env):
    ...
    def __init__(self, 
                 ...
                 initial_exploration_radius: float = 10.0,
                 max_exploration_radius: float = 50.0,
                 radius_increase_episodes: int = 10000):
        super().__init__()
        ...
        self.initial_exploration_radius = initial_exploration_radius
        self.max_exploration_radius = max_exploration_radius
        self.radius_increase_episodes = radius_increase_episodes
        self.current_exploration_radius = initial_exploration_radius
        ...
        # Dynamic goal position (will be set in reset)
        self.goal_pos = None
        ...
        print(f"[INFO] Mountain Pass Exploratory Environment initialized")
        print(f"[INFO] Initial exploration radius: {self.initial_exploration_radius}m")
        print(f"[INFO] Max exploration radius: {self.max_exploration_radius}m")
        print(f"[INFO] Radius increases every {self.radius_increase_episodes} episodes")
    
    def generate_random_goal(self, current_position: airsim.Vector3r) -> airsim.Vector3r:
        """Generate a random goal within the current exploration radius."""
        # Generate random angle and distance within the radius
        angle = np.random.uniform(0, 2 * np.pi)
        distance = np.random.uniform(5.0, self.current_exploration_radius)  # Minimum 5m distance
        
        # Calculate goal position relative to current position
        goal_x = current_position.x_val + distance * np.cos(angle)
        goal_y = current_position.y_val + distance * np.sin(angle)
        
        # Generate random altitude within safe bounds
        # Keep altitude similar to current position with some variation
        altitude_variation = np.random.uniform(-3.0, 3.0)
        goal_z = current_position.z_val + altitude_variation
        
        # Ensure altitude is within bounds
        goal_z = max(goal_z, -self.max_altitude)  # AirSim uses negative Z for up
        goal_z = min(goal_z, -self.min_altitude)
        
        goal_pos = airsim.Vector3r(goal_x, goal_y, goal_z)
        
        print(f"[EXPLORATION] Generated goal at ({goal_x:.2f}, {goal_y:.2f}, {goal_z:.2f})")
        print(f"[EXPLORATION] Distance from current: {distance:.2f}m, Angle: {angle*180/np.pi:.1f}°")
        print(f"[EXPLORATION] Current exploration radius: {self.current_exploration_radius:.1f}m")
        
        return goal_pos
    
    def update_exploration_radius(self):
        """Update exploration radius based on episode count."""
        # Calculate how many radius increases should have occurred
        radius_increases = self.total_episodes // self.radius_increase_episodes
        
        # Calculate new radius
        new_radius = self.initial_exploration_radius + (radius_increases * 5.0)  # Increase by 5m each time
        ...
        if new_radius != self.current_exploration_radius:
            ...
            print(f"[EXPLORATION] Total episodes: {self.total_episodes}, Radius increases: {radius_increases}")
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Update exploration radius based on episode count
        self.update_exploration_radius()
        ...
        # Get current position and generate new random goal
        state = self.client.getMultirotorState()
        current_pos = state.kinematics_estimated.position
        self.goal_pos = self.generate_random_goal(current_pos)
    ...
    def step(self, action):
        ...
        info['exploration_radius'] = self.current_exploration_radius
    ...
    def _check_termination(self) -> Tuple[bool, bool, Dict[str, Any]]:
        ...
        info = {
            ...
            'exploration_radius': self.current_exploration_radius,
...
def main():
    """Test the exploratory environment."""
    print("[INFO] Testing Mountain Pass Exploratory Environment...")
